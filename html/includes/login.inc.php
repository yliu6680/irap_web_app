...
if (isset($_POST['login-submit'])) {
  ...
  if (empty($mailuid) || empty($password)) {
    header("Location: ../irap_login.php?error=emptyfields&UsrName=".$mailuid);
    exit();
  }
  else {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      header("Location: ../irap_login.php?error=sqlerror");
      exit();
    }
    else {
      ...
      if ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($pwdCheck == false) {
          // If there is an error we send the user back to the signup page.
          header("Location: ../irap_login.php?error=wrongpwd");
          exit();
        }
        // Then if they DO match, then we know it is the correct user that is trying to log in!
          ...
          $_SESSION['usrname'] = $row['uidUsers'];
          $_SESSION['email'] = $row['emailUsers'];
	  $cmd1 = shell_exec("python3 /var/www/script/generate_dir.py ".$_SESSION['usrname']." 2>&1");
          // add something to initial directories
        ...
        header("Location: ../irap_login.php?login=wronguidpwd");
        exit();
      }
    }
  }
