...
if (isset($_POST['signup-submit'])) {
  ...
  if (empty($username) || empty($email) || empty($password) || empty($passwordRepeat)) {
    header("Location: ../irap_signup.php?error=emptyfields&uid=".$username."&mail=".$email);
    exit();
  }
  // We check for an invalid username AND invalid e-mail.
  else if (!preg_match("/^[a-zA-Z0-9]*$/", $username) && !filter_var($email, FILTER_VALIDATE_EMAIL)) {
    header("Location: ../irap_signup.php?error=invaliduidmail");
    exit();
  }
  // We check for an invalid username. In this case ONLY letters and numbers.
  else if (!preg_match("/^[a-zA-Z0-9]*$/", $username)) {
    header("Location: ../irap_signup.php?error=invaliduid&mail=".$email);
    exit();
  }
  ...
  // We check if the repeated password is NOT the same.
    ...
    header("Location: ../irap_signup.php?error=passwordcheck&uid=".$username."&mail=".$email);
    exit();
  }
  else {
    $command="aws ses verify-email-identity --email-address " . $email;      
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      header("Location: ../irap_signup.php?error=sqlerror");
      exit();
    }
    else {
      ...
      if ($resultCount > 0) {
        header("Location: ../irap_signup.php?error=usertaken&mail=".$email);
        exit();
      }
      else {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          ...
          header("Location: ../irap_signup.php?signup=success");
          exit();
  ...
  header("Location: ../irap_signup.php");
  exit();
}
